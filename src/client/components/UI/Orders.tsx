...
const StationIcon: React.FC<{ type: InteractType, size?: number, className?: string }> =
    ({ type, size = 14, className = "" }) => {
        const iconProps = { size, strokeWidth: 2.5, className: `flex-shrink-0 ${className}` };
        switch (type) {
            case InteractType.Oven: return <Flame {...iconProps} className="text-orange-500" />;
            case InteractType.ChoppingBoard: return <Slice {...iconProps} className="text-green-500" />;
            case InteractType.Fridge:
            case InteractType.Stock: return <Inbox {...iconProps} className="text-blue-500" />;
            default: return <CookingPot {...iconProps}  />;
        }
    };
...
const OrderCard: React.FC<OrderCardProps> = ({ order }) => {
    ...
    useEffect(() => {
        ...
        if (order.deadline > 0 && order.deadline > Date.now()) {
            const intervalId = setInterval(() => {
                ...
                if (newTimeInfo.timeLeft <= 0) { clearInterval(intervalId); }
            }, 1000);
        ...
        steps.forEach(step => {
            if (step.type === 'GET') {
                ...
                step.requiredIngredients?.forEach(req => {
                    ...
                    if (!processedOutputs.has(req) &&
                        (step.stationType === InteractType.ChoppingBoard || step.stationType === InteractType.Oven))
                    {
                        const exists = info.ingredientsToProcess.some(
                            p => p.ingredient === req && p.station === step.stationType
                        );
                        if (!exists) {
    ...
    return (
        ...
        <div className={`bg-gradient-to-br from-purple-300 via-pink-200 to-yellow-100 rounded-lg shadow-md px-3 py-2.5 w-full flex flex-col gap-2 transition-colors duration-300 ${timeInfo.isUrgent ? 'animate-pulse ring-2 ring-red-500 ring-offset-1 ring-offset-purple-300/50' : ''} ${timeInfo.isExpired ? 'opacity-60 grayscale' : ''}`}>

            {/* Top part: Final item icon + name */}
            <div className="flex items-center gap-2.5">
                <div className="flex-shrink-0 w-11 h-11 bg-white/80 rounded-md overflow-hidden flex items-center justify-center shadow border border-white/30"> {/* Slightly larger icon */}
                    <img src={finalIconUrl} alt={finalRecipe.name} className="object-contain w-9 h-9" onError={(e) => (e.currentTarget.src = '/icons/placeholder.png')} />
                </div>
            ...
            <div className="flex flex-col gap-1.5 bg-black/10 p-1.5 rounded-md mt-1">
                ...
                {processedInfo.finalAssemblyIngredients.length > 0 && (
                    <div className="flex items-center gap-x-1.5 gap-y-1 flex-wrap">
                        ...
                        {processedInfo.finalAssemblyStation && (
                            <StationIcon
                                type={processedInfo.finalAssemblyStation}
                                size={16}
                                className="text-green-700 opacity-90 mr-1"
                            />
                        )}
                        {processedInfo.finalAssemblyIngredients.map((ing, index) => (
                            <IngredientIcon
                                ...
                                isResult={processedInfo.ingredientsToProcess.some(p => p.ingredient === ing) || getItemDefinition(ing)?.isResult || finalRecipe.result.ingredient === ing}
                            />
                        ))}
                ...
                {processedInfo.ingredientsToProcess.length === 0 && processedInfo.finalAssemblyIngredients.length === 0 && processedInfo.allBaseIngredients.length > 0 && (
                    <div className="flex items-center gap-1 flex-wrap text-xs text-gray-600 italic">
                        (Requires: {processedInfo.allBaseIngredients.map((ing, i) => <IngredientIcon key={i} ingredient={ing} sizeClass="w-4 h-4"/>)} )
                    </div>
                )}
            ...
            {timeInfo.totalTime > 0 && (
                <div className="flex items-center gap-1.5 w-full mt-2">
                    <Clock size={13} className={`flex-shrink-0 ${timeInfo.isUrgent ? 'text-red-700' : 'text-gray-700'} ${timeInfo.isExpired ? 'text-gray-500' : ''}`}/>
...
export default function Orders() {
    ...
    return (
        <div className="fixed top-4 right-2 z-10 w-72 max-h-[calc(100vh-8rem)] overflow-y-auto overflow-x-hidden p-2 bg-black/40 backdrop-blur-sm rounded-lg shadow-lg flex flex-col">
            <h3 className="text-center font-semibold text-white/90 text-sm mb-2 flex items-center justify-center gap-1 flex-shrink-0"><ChefHat size={16} /> Orders</h3>
            <div className="flex flex-col gap-2">
                <AnimatePresence initial={false}>
                    {sortedOrders.length > 0 ? (
                        sortedOrders.map((order) => (
                            <motion.div key={order.id} /* ... animation props ... */
                                        layout
