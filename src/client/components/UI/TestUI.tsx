  ...
  return (
    <div className="flex flex-col bg-base-200 text-base-content h-screen">
      ...
      <div className="tabs tabs-boxed bg-base-300">
        <a onClick={() => setCurrentTab('chat')} className={`tab ${currentTab === 'chat' ? 'tab-active' : ''}`}>
          Chat
        </a>
        <a onClick={() => setCurrentTab('players')} className={`tab ${currentTab === 'players' ? 'tab-active' : ''}`}>
          Players
        </a>
      </div>
        ...
        {currentTab === 'chat' ? (
          <div className="flex flex-col h-full">
            <div className="flex-1 overflow-y-auto space-y-2">
              {messages.map((message, index) => (
                <div key={index} className={`chat ${room?.sessionId === message.split(' ')[0].slice(1, -1) ? 'chat-end' : 'chat-start'}`}>
          ...
          <div className="flex flex-col h-full overflow-y-auto space-y-2">
            {[...(players?.values() ?? [])].map((player, index) => (
              <div key={index} className="card card-bordered bg-base-100 p-2 shadow-sm">
                <p>{player.name}</p>
              </div>
            ))}
          </div>
